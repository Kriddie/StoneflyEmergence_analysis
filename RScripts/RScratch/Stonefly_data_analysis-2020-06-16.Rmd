...
library(dplyr)
library(tidyr)
df <- read.csv(here::here("/Emergence_PickedList_2020-04-23.csv"))
...
pivot <- df2 %>%
  group_by(RiverCode, FloodplaneCode,SiteCode,genus_species)%>%        #select wich variable you want to group by
...
row.names(pivot2) <- pivot2$SiteCode
pivot2[1] <- NULL


pivot2 <-as.data.frame(pivot2)
#names <- colnames(pivot2)
#pivot2 <- InsertRow(pivot2, names, RowNum = 1)
#colnames(pivot2) <- NULL
...
plot(stoneflyDF.spp_NMS)
orditorp(stoneflyDF.spp_NMS, "sites")   # Gives points labels
