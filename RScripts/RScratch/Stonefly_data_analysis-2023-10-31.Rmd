...
The folowing two plots are stress plots and NMDS plots respectively. The following NMDS plot compares species composition at sites by combining all species collected at each site
...
ordiplot(stoneflyDF.spp_NMS,type="n")
orditorp(stoneflyDF.spp_NMS,display="sites",cex=1.25,air=0.01)
orditorp(stoneflyDF.spp_NMS,display="species",col="red",air=0.01)
...
ordiplot(stoneflyDF.spp_NMS,type="n")
ordihull(stoneflyDF.spp_NMS,groups=treat,draw="polygon",col="grey90",label=F)
#orditorp(stoneflyDF.spp_NMS,display="species",col="grey20",air=0.01)
orditorp(stoneflyDF.spp_NMS,display="sites",col=c(rep("green",5),rep("blue",5),rep("red",7)),
         air=0.01,cex=1.25)
...
treat=c(rep("KA",5),rep("MF",4),rep("NF",8))
ordiplot(stoneflyDF.spp_NMS,type="n")
ordihull(stoneflyDF.spp_NMS,groups=treat,draw="polygon",col="grey90",label=F)
orditorp(stoneflyDF.spp_NMS,display="species",col="grey20",air=0.01)
orditorp(stoneflyDF.spp_NMS,display="sites",col=c(rep("green",8),rep("blue",5),rep("red",7)),
         air=0.01,cex=1.25)


example_NMDS <- metaMDS(community_matrix, # Our community-by-species matrix
                     k=2, trace = 0) # The number of reduced dimensions
data.scores <- as.data.frame(scores(stoneflyDF.spp_NMS))
data.scores$site <- rownames(data.scores)
data.scores$grp <- c(rep("KA",5),rep("MF",4),rep("NF",8))
species.scores <- as.data.frame(scores(stoneflyDF.spp_NMS, "species"))
species.scores$species <- rownames(species.scores)

ggplot() + 
  geom_text(data=stoneflyDF.spp_NMS,aes(x=NMDS1,y=NMDS2,label=species), alpha=1) +  # add the species labels
