...
pivot <- df %>%
  select(FloodplaneCode,SiteCode, Species,count) %>%  #select variabls
...
```{r, message=FALSE, warning=FALSE, test.data, echo=FALSE}
...
test.NMDS=metaMDS(abundance.matrix, # Our community-by-species matrix
                  k=2) # The number of reduced dimensions
stressplot(test.NMDS)
plot(test.NMDS)
